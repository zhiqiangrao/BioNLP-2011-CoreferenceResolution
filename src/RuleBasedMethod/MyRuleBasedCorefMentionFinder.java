...
public class MyRuleBasedCorefMentionFinder implements CorefMentionFinder {
...
//  protected int maxID = -1;
  ...
  public MyRuleBasedCorefMentionFinder() {
    this(Constants.ALLOW_REPARSING);
  }

  public MyRuleBasedCorefMentionFinder(boolean allowReparsing) {
...
//    this.maxID = _maxID;
  ...
  protected void findHead2(CoreMap s, Mention m, int begin, int end) {
	  Tree tree = s.get(TreeCoreAnnotations.TreeAnnotation.class);
	  List<CoreLabel> sent = s.get(CoreAnnotations.TokensAnnotation.class);
	  tree.indexSpans(0);
    
      Tree head = findSyntacticHead2(m, tree, sent, begin, end);
      int headIndex = ((CoreLabel) head.label()).get(CoreAnnotations.IndexAnnotation.class)-1;
      String headstr = ((CoreLabel) head.label()).get(CoreAnnotations.TextAnnotation.class);
      int headidx = -1;
      for (CoreLabel token : sent) {
    	  if (token.get(CoreAnnotations.IndexAnnotation.class)-1 < m.startIndex)
  				continue;
  		if (token.get(CoreAnnotations.TextAnnotation.class).equals(headstr)) {
  			if (headidx == -1)
  				headidx = token.get(CoreAnnotations.IndexAnnotation.class)-1;
  			else if(Math.abs(token.get(CoreAnnotations.IndexAnnotation.class)-1-headIndex) < Math.abs(headidx-headIndex))
  				headidx = token.get(CoreAnnotations.IndexAnnotation.class)-1;
  		}
	  }
      // head word的长度大于token
      if (headidx == -1) {
		  for (CoreLabel token : sent) {
			  if (token.get(CoreAnnotations.IndexAnnotation.class)-1 < m.startIndex)
	  				continue;
				if (headstr.contains(token.get(CoreAnnotations.TextAnnotation.class))) {
					if (headidx == -1)
		  				headidx = token.get(CoreAnnotations.IndexAnnotation.class)-1;
		  			else if(Math.abs(token.get(CoreAnnotations.IndexAnnotation.class)-1-headIndex) < Math.abs(headidx-headIndex))
		  				headidx = token.get(CoreAnnotations.IndexAnnotation.class)-1;
  ...
  protected Tree findSyntacticHead2(Mention m, Tree root, List<CoreLabel> tokens, int begin, int end) {
	    // mention ends with 's
	    ...
	    Tree exactMatch = findTreeWithSpan(root, begin, end);
	    //
	    // found an exact match
	    //
	    if (exactMatch != null) {
	      //return safeHead(exactMatch, endIdx);
	    	return safeHead(exactMatch, end);
	    }
  ...
  protected static boolean isPleonastic(Mention m, Tree tree) {
